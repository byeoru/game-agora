...
export default () => {
  const [orderOfPopularity, setOrderOfPopularity] = useState({
    loading: true,
    PointAndClick: [],
    Fighting: [],
    Shooter: [],
  ...
  const getData = async () => {
    const [
      orderOfRatingQuery,
      orderOfRatingError,
    ] = await igdbApi.genrePopularityMultiQuery(getDayAgoNowSec(200));
    setOrderOfPopularity({
      loading: false,
  ...
  return <TopNavSharingPresenter {...orderOfPopularity} />;
};
